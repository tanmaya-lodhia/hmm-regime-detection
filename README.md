...
- Model: `GaussianHMM` with full covariance, where each state gets its own covariance matrix so the model captures how return and vol co-move within a regime, not just independently
